document.addEventListener('DOMContentLoaded', function() {
    ...
    closeModalButton.addEventListener('click', function() {
        if (packageId) {
            // For package bookings, show warning and redirect
            if (confirm('Are you sure you want to cancel the package booking? This will terminate your current booking process.')) {
                window.location.href = '/booking';  // Redirect to booking page
            }
        } else {
            // Regular booking - just close the modal
    ...
    overlay.addEventListener('click', function(event) {
        // Only close if it's not a package booking
        if (!packageId) {
    ...
    // Get URL parameters for package booking
    const urlParams = new URLSearchParams(window.location.search);
    const packageId = urlParams.get('package_id');
    const numPeople = urlParams.get('num_people');
    
    // Set maxSeats based on whether it's a package booking or regular booking
    const maxSeats = packageId ? parseInt(numPeople) : 8;
        ...
        if (e.target.classList.contains('seat') || e.target.closest('.seat')) {
            ...
            if (seatElement.classList.contains('selected')) {
                ...
                if (selectedSeats.length < maxSeats) {
                    ...
                    if (packageId) {
                        alert(`You can only select ${maxSeats} seat${maxSeats !== 1 ? 's' : ''} as per your package selection.`);
                    } else {
                        showMaxSeatsAlert();
                    }
                    return;
    ...
    function updateSeatInfo() {
        ...
        // Update confirm button state based on booking type
        if (packageId) {
            // For package bookings: enable only when exact number of seats are selected
            confirmButton.disabled = selectedSeats.length !== parseInt(numPeople);
            
            // Update message to show progress
            const remainingSeats = parseInt(numPeople) - selectedSeats.length;
            if (remainingSeats > 0) {
                selectedSeatsElement.textContent += ` (${remainingSeats} more seat${remainingSeats !== 1 ? 's' : ''} required)`;
            }
        } else {
            // For regular bookings: enable when at least one seat is selected
    ...
    confirmButton.addEventListener('click', function() {
        ...
        const bookingDetails = {
            ...
            bus_amount: currentBusTicketPrice * selectedSeats.length
        };

        // Add package details if this is a package booking
        if (packageId) {
            bookingDetails.package_id = packageId;
            bookingDetails.package_amount = parseFloat(urlParams.get('package_amount'));
            bookingDetails.num_people = parseInt(numPeople);
            // Calculate total amount including package
            const packageTotal = bookingDetails.package_amount * bookingDetails.num_people;
            const busTotal = currentBusTicketPrice * selectedSeats.length;
            bookingDetails.total_amount = packageTotal + busTotal;
        }
    ...
    function formatDate(dateElement) {
        if (dateElement) {
            ...
            // Handle "DD Mon Day" format (e.g., "13 Feb Thu")
    ...
    if (packageId && numPeople) {
        // Find the view seats button for this bus
        const viewSeatsBtn = document.querySelector(`.view_seat_btn[data-bus-id="${busId}"]`);
        if (viewSeatsBtn) {
            // Set the current bus ticket price from the button's data attribute
            currentBusTicketPrice = parseFloat(viewSeatsBtn.dataset.ticketPrice);
            currentBusId = busId;
