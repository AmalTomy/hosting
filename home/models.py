...
class Users(AbstractUser):
    ...
    is_police = models.BooleanField(default=False, null=True, blank=True)
...
class Bus(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.update_status()  # Add this line to automatically update status
...
class Police(models.Model):
    user = models.OneToOneField(Users, on_delete=models.CASCADE, related_name='police_profile', null=True, blank=True)
    policestation = models.CharField(max_length=100, null=True, blank=True)
    address = models.CharField(max_length=255, null=True, blank=True)
    location = models.CharField(max_length=100, null=True, blank=True)
    phnnum = models.CharField(max_length=20, null=True, blank=True)
    status = models.CharField(max_length=20, default='active', null=True, blank=True)

    def __str__(self):
        return f"{self.policestation} - {self.location}"
...
class MissingPerson(models.Model):
    ...
    missing_from = models.DateField(null=True, blank=True, default=timezone.now)
    location = models.CharField(max_length=100, null=True, blank=True)  # Added location field
    reported_by = models.ForeignKey(Users, on_delete=models.CASCADE, related_name='reported_cases')
    investigating_police = models.ForeignKey(Police, on_delete=models.SET_NULL, null=True, blank=True, related_name='investigating_cases')
...
class Detected(models.Model):
    case = models.ForeignKey(MissingPerson, on_delete=models.CASCADE, related_name='detections', null=True, blank=True)
    ...
    def __str__(self):
        return f"Detection for {self.case.first_name} at {self.location}"
...
class TourPackage(models.Model):
    ...
    bus = models.ForeignKey(Bus, on_delete=models.CASCADE, related_name='packages', null=True, blank=True)
    ...
    # Features
    has_bus = models.BooleanField(default=False, null=True, blank=True)
    has_hotel = models.BooleanField(default=False, null=True, blank=True)
    has_meals = models.BooleanField(default=False, null=True, blank=True)
    hotel_category = models.CharField(max_length=1, choices=HOTEL_CATEGORY_CHOICES, null=True, blank=True)

    # Images
    main_image = models.ImageField(upload_to='package_images/', null=True, blank=True)
...
class PackageImage(models.Model):
    package = models.ForeignKey(TourPackage, related_name='gallery_images', on_delete=models.CASCADE,null=True, blank=True)
...
class PackageDay(models.Model):
    ...
    package = models.ForeignKey(TourPackage, related_name='days', on_delete=models.CASCADE, null=True, blank=True)
    day_number = models.PositiveIntegerField(null=True, blank=True)
    title = models.CharField(max_length=200, null=True, blank=True)
    meals = models.CharField(max_length=20, choices=MEAL_CHOICES, blank=True, null=True)
    ...
    def __str__(self):
        return f"Day {self.day_number} of {self.package.title}"

class PackageActivity(models.Model):
    day = models.ForeignKey(PackageDay, on_delete=models.CASCADE, related_name='activities',null=True, blank=True)
    ...
    def __str__(self):
        return f"{self.day.package.title} - Day {self.day.day_number} - {self.title}"

class PackagePolicy(models.Model):
    package = models.OneToOneField(TourPackage, related_name='policy', on_delete=models.CASCADE, null=True, blank=True)
    cancellation_policy = models.TextField(null=True, blank=True)
    terms_conditions = models.TextField(null=True, blank=True)

    def __str__(self):
        return f"Policies for {self.package.title}"
...
class PackageBooking(models.Model):
    ...
    package_booking_id = models.AutoField(primary_key=True)
    ...
    bus_booking = models.ForeignKey('BusBooking', on_delete=models.SET_NULL, null=True, blank=True)  # Reference to bus booking
    num_travelers = models.IntegerField(null=True, blank=True)
    seat_numbers = models.CharField(max_length=255, null=True, blank=True)
    ...
    passenger_details = models.JSONField(null=True, blank=True)
    special_requests = models.TextField(null=True, blank=True)
    schedule_version = models.IntegerField(default=1, null=True, blank=True)

    def __str__(self):
        return f"Package Booking {self.package_booking_id} - {self.customer}"
