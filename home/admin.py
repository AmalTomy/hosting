...
from .models import Users, Moderator, Customers, Bus, Location, BusBooking, Payment, TravelReport, ReportPhoto, Agent, AgentJob, SafetyNotificationReport, SafetyReportMedia, ChatMessage, DriversInfo, Feedback, BusReschedule, Police, MissingPerson, Detected, TourPackage, PackageDay, PackageActivity, PackageImage, PackageBooking
...
class PoliceAdmin(admin.ModelAdmin):
    list_display = ('policestation', 'location', 'phnnum', 'status')
    list_filter = ('status', 'location')
    search_fields = ('policestation', 'location', 'phnnum')
    raw_id_fields = ('user',)

class MissingPersonAdmin(admin.ModelAdmin):
    list_display = ('first_name', 'last_name', 'gender', 'date_reported', 'status', 'is_finished')
    list_filter = ('status', 'gender', 'is_finished')
    search_fields = ('first_name', 'last_name', 'address')
    date_hierarchy = 'date_reported'
    raw_id_fields = ('reported_by', 'investigating_police')
    readonly_fields = ('date_reported',)
    
class DetectedAdmin(admin.ModelAdmin):
    list_display = ('case', 'location', 'timestamp', 'confidence_score')
    list_filter = ('timestamp',)
    search_fields = ('location', 'case__first_name', 'case__last_name')
...
class TourPackageAdmin(admin.ModelAdmin):
    list_display = ('title', 'category', 'price', 'hotel_category', 'has_bus', 'has_hotel', 'has_meals')
    search_fields = ('title',)
    list_filter = ('category', 'hotel_category', 'has_bus', 'has_hotel', 'has_meals')
...
class PackageActivityAdmin(admin.ModelAdmin):
    list_display = ('day', 'title', 'description')
    search_fields = ('day__package__title',)
    raw_id_fields = ('day',)
    
class PackageImageAdmin(admin.ModelAdmin):
    list_display = ('package', 'image', 'caption')
    search_fields = ('package__title',)
    raw_id_fields = ('package',)


class PackageBookingAdmin(admin.ModelAdmin):
    list_display = ('package', 'num_travelers', 'total_amount', 'bus', 'bus_booking', 'customer', 'booking_date','seat_numbers')
    search_fields = ('package__title',)
    raw_id_fields = ('package',)
...
admin.site.register(PackageDay, PackageDayAdmin)
admin.site.register(PackageActivity, PackageActivityAdmin)
admin.site.register(PackageImage, PackageImageAdmin)
admin.site.register(PackageBooking, PackageBookingAdmin)